...
//Function prototypes
static void collision_movexy(RvR_port_cam *cam, RvR_fix22 vel[3], RvR_fix22 *vx, RvR_fix22 *vy);
static void collision_movez(RvR_port_cam *cam, RvR_fix22 vel[3]);
...
void collision_move(RvR_port_cam *cam, RvR_fix22 vel[3])
{
   if(cam==NULL)
      return;
   ...
   for(int i = 0; i<2; i++)
   {
      ...
      nvx = vel[0];
      nvy = vel[1];
      collision_movexy(cam, vel, &nvx, &nvy);
      if(i==0)
      {
         vx = nvx;
         vy = nvy;
      ...
      RvR_fix22 dx = cam->x - ox;
      RvR_fix22 dy = cam->y - oy;
   ...
   if(!RvR_key_down(RVR_KEY_P))
      collision_movez(cam,vel);
   vel[0] = vx;
   vel[1] = vy;

   vel[0] = RvR_fix22_mul(vel[0], 856);
   vel[1] = RvR_fix22_mul(vel[1], 856);
}
...
static void collision_movexy(RvR_port_cam *cam, RvR_fix22 vel[3], RvR_fix22 *vx, RvR_fix22 *vy)
{
   ...
   RvR_fix22 mag = RvR_fix22_sqrt64((int64_t)(vel[0]) * (int64_t)(vel[0]) + (int64_t)(vel[1]) * (int64_t)(vel[1])) / 32;
   ...
   if(!RvR_key_down(RVR_KEY_C))
      RvR_array_push(collision_sector_stack, cam->sector);
   for(int i = 0; i<RvR_array_length(collision_sector_stack); i++)
   {
      uint16_t sector = collision_sector_stack[i];

      for(int j = 0; j<map->sectors[sector].wall_count; j++)
      {
         RvR_port_wall *wall = map->walls + map->sectors[sector].wall_first + j;
         ...
         p1_org[0] = p1[0] = map->walls[wall->p2].x;
         p1_org[1] = p1[1] = map->walls[wall->p2].y;
         ...
         if(!line_inter)
         {
            ...
            if(dist_min>=RvR_fix22_mul(256, 256))
               continue;
         ...
         if(wall->portal!=RVR_PORT_SECTOR_INVALID)
         {
            ...
            RvR_fix22 cp_inv[2];
            cp_inv[0] = (RvR_fix22)(((int64_t)cp[0] * v[0] - (int64_t)cp[1] * v[1]) / RvR_non_zero(mag) + cam->x);
            cp_inv[1] = (RvR_fix22)(((int64_t)cp[0] * v[1] + (int64_t)cp[1] * v[0]) / RvR_non_zero(mag) + cam->y);
            RvR_fix22 floor = RvR_port_sector_floor_at(map, wall->portal, cp_inv[0], cp_inv[1]);
            RvR_fix22 ceiling = RvR_port_sector_ceiling_at(map, wall->portal, cp_inv[0], cp_inv[1]);

            if(cam->z + 256>=floor&&cam->z + 128<=ceiling&&ceiling - floor>=128)
            {
               continue;
            }
         }
         ...
         {
            RvR_fix22 root = RvR_fix22_sqrt64(((int64_t)256* 256) - (int64_t)(cp[1] - p0[1]) * (int64_t)(cp[1] - p0[1])) / 32;
            nx = p0[0] - root - 2;

            //TODO(Captain4LK): maybe only do this projection if the closest point was p0/p1, not the resolving point
            if(nx<=min_p[0])
               collision_project(vel[0], vel[1], (RvR_fix22)(-(p0_org[1] - (cam->y + ((int64_t)vel[1]*nx)/RvR_non_zero(mag)))),
                                 (RvR_fix22)(p0_org[0] - (cam->x + ((int64_t)vel[0]*nx)/RvR_non_zero(mag))), &nv[0], &nv[1]);
         }
         else if((p1[1] - p0[1]>=0&&rp[1] - p0[1]>p1[1] - p0[1])||
                 (p1[1] - p0[1]<0&&rp[1] - p0[1]<p1[1] - p0[1]))
         {
            RvR_fix22 root = RvR_fix22_sqrt64((int64_t)256* (int64_t)256- (int64_t)(cp[1] - p1[1]) * (int64_t)(cp[1] - p1[1])) / 32;
            nx = p1[0] - root - 2;

            if(nx<=min_p[0])
               collision_project(vel[0], vel[1], (RvR_fix22)(-(p1_org[1] - (cam->y + ((int64_t)vel[1]*nx)/RvR_non_zero(mag)))),
                                 (RvR_fix22)(p1_org[0] - (cam->x + ((int64_t)vel[0]*nx)/RvR_non_zero(mag))), &nv[0], &nv[1]);
         }
         else
         {
            ...
            if(nx<=min_p[0])
               collision_project(vel[0], vel[1], p1_org[0] - p0_org[0], p1_org[1] - p0_org[1], &nv[0], &nv[1]);
         }
   ...
   *vy = nv[1];
   ...
   cam->x+=(RvR_fix22)(((int64_t)vel[0] * min_p[0]) / RvR_non_zero(mag));
   cam->y+=(RvR_fix22)(((int64_t)vel[1] * min_p[0]) / RvR_non_zero(mag));
   cam->sector = RvR_port_sector_update(map, cam->sector, cam->x, cam->y);
...
static void collision_movez(RvR_port_cam *cam, RvR_fix22 vel[3])
{
   ...
   RvR_fix22 floor = RvR_port_sector_floor_at(map, cam->sector, cam->x, cam->y);
   RvR_fix22 ceiling = RvR_port_sector_ceiling_at(map, cam->sector, cam->x, cam->y);
   ...
   for(int i = 0; i<RvR_array_length(collision_sector_stack); i++)
   {
      ...
      for(int j = 0; j<map->sectors[sector].wall_count; j++)
      {
         ...
         p0[0] = wall->x;
         p0[1] = wall->y;
         p1[0] = map->walls[wall->p2].x;
         p1[1] = map->walls[wall->p2].y;

         RvR_fix22 proj[2];
         RvR_fix22 dist2 = collision_point_segment_dist2(p, p0, p1, proj);
         if(dist2>=RvR_fix22_mul(256,256))
            continue;
         ...
         if(wall->portal!=RVR_PORT_SECTOR_INVALID)
         {
            ...
            RvR_fix22 sfloor = RvR_port_sector_floor_at(map, wall->portal, proj[0], proj[1]);
            RvR_fix22 sceiling = RvR_port_sector_ceiling_at(map, wall->portal, proj[0], proj[1]);
            if(cam->z + 256>=sfloor&&cam->z + 128<=sceiling&&sceiling - sfloor>=128)
            {
   ...
   cam->z = RvR_min(ceiling - 128, RvR_max(cam->z + vel[2] / 64, floor));
}
