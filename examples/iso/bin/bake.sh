...
for f in ../assets/png/*.png; do
   if [ `basename $f .png` -ge 16384 ]; then
      ../../../tools/texture-tool/texture-tool --in $f --pal ../assets/aurora.pal --out data/tex`basename $f .png`.rvr;
   else
      ../../../tools/texture-tool/texture-tool --in $f --wall --pal ../assets/aurora.pal --out data/tex`basename $f .png`.rvr;
   fi
