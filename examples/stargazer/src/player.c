...
{
   player.entity->col_height = 1024;
   player.entity->col_radius = 384;
   player.entity->cards_size = 71;
   player.entity->cards = RvR_malloc(sizeof(*player.entity->cards)*player.entity->cards_size);
...
void player_update()
{
   ...
   {
      player.entity->vel.x+=direction.x;
      player.entity->vel.y+=direction.y;
   }
   ...
   {
      player.entity->vel.x+=direction.y;
      player.entity->vel.y-=direction.x;
   }
   ...
   if(RvR_core_key_pressed(config_use))
   ...
   if(RvR_core_key_pressed(config_jump)&&player.entity->on_ground)
   {
      //sound_play(SOUND_PLAYER_JUMP,ai_index_get(player.entity),255);
      player.entity->vel.z = JUMP_SPEED;
   }
   ...
   //Cap player speed
   player.entity->vel.z = RvR_max(-MAX_VERTICAL_SPEED,RvR_min(player.entity->vel.z,MAX_VERTICAL_SPEED));
   //-------------------------------------
   ...
   if(player.entity->pos.z+player.entity->col_height>=ceiling_height)
      player.entity->vel.z = 0;
